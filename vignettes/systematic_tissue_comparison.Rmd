...
And load in the dataset -- this R data file can be found [here]().

```{r, eval=F, echo=F}
df <- readRDS("~/Google Drive/scRNA_metabolism_paper/Paper/Fig6/Disease_data/CD4_healthy_tissues_Sims_GSE126030/biowulf_comparisons/tissue_final_doublets.rds")
```

```{r, eval=F}
tissue_data <- readRDS("t_cell_tissue_data.rds")
```
...
We can now plot a broad summary using the qvals from each comparison. For this, we're just going to represent the data in a heatmap, using the cool ComplexHeatmap package.
```{r, eval=F}
hm <- all_data %>%
  Heatmap(name = "Qval",
          show_row_names = F, 
...
And after going back to the pathway gene expression, we can see that a large effect of this pathway signature was driven by expression of alpha defensin proteins, exclusively in T cells from the bone marrow
