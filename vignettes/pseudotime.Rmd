...
Here we're going to look at tracking gene set changes across a pseudotime trajectory, using the multisample capability of SCPA. To do this, we'll use a dataset that we generated where naive CD4 T cells were left unstimulated, or stimuated for 12 or 24 hours with anti-CD3 and anti-CD28. You can find this dataset [here](). We'll do a systematic analysis of how metabolic pathways are transcriptionally regulated throughout naive CD4^+^ T cell activation.

Let's load in a few packages to start

```{r setup, results='hide', warning=FALSE, message=FALSE, eval=F}
library(SCPA)
library(Seurat)
library(tidyverse)
library(magrittr)
...
### Extract groups based on milestones and add information to Seurat object
...
pathways taken from Hallmark, KEGG, and Reactome databases that you can find [here](https://github.com/jackbibby1/SCPA/blob/main/gene_sets/.csv)
